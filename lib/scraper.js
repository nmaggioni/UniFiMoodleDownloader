...
async function scrapeCourse(page, courseId) {
  ...
  for (let section of sections) {
    ...
    logger.debug(`   └─ Trovata sezione "${sectionName}"`);
    ...
    for (let resource of resources) {
      ...
      let resourceName = await resource.$eval('a span.instancename', el => el.childNodes[0].nodeValue);
      let resourceViewUrl = await resource.$eval('a', el => el.href);
      logger.debug(`      └─ Trovata risorsa "${resourceName}" @ "${resourceViewUrl}"`);
      sectionResources[resourceName] = resourceViewUrl;
    }
    sectionContents[sectionName] = sectionResources;
    ...
    for (let [i, resource] of altResources.entries()) {
      ...
      if (resourceViewUrl) {
        ...
        altSectionResources[`[ALT#${i}] ${resourceName}`] = resourceViewUrl;
      }
    }
    Object.assign(sectionContents[sectionName], altSectionResources);
  }
